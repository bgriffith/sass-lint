...
var isPseudoType = function (type, node) {
  if (node.contains('pseudoClass')) {
    var pseudoClass = node.first('pseudoClass');
 ...
 *
 * @param {Object} node - The current node to run the checks against
 * @returns {Object} The results of the rule checks
 */
var runRuleChecks = function (node) {
  var checks = {};

  // Loop through the nodes rulesets
  node.forEach('ruleset', function (child) {

    // The loop through the selectors of each ruleset
    child.forEach('selector', function (selector) {
      if (isFocus(selector)) {
